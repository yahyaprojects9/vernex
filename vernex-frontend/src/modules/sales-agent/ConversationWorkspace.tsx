...
import { Archive, Paperclip, Pin, Send } from "lucide-react";
...
export function ConversationWorkspace() {
  ...
  function sendMessage() {
    ...
    const staffMessage = { id: `MSG-${Date.now()}`, sender: "staff" as const, body: message, time: "Now" };
    ...
    <div className="grid gap-4 xl:grid-cols-[320px_1fr_320px]">
      <aside className="dashboard-surface overflow-hidden">
        <div className="space-y-3 border-b border-border p-4">
          <Input placeholder="Create conversation" value={newCustomer} onChange={(event) => setNewCustomer(event.target.value)} />
          <Button className="w-full" onClick={createConversation}>Create Conversation</Button>
      ...
      <section className="dashboard-surface flex min-h-[680px] flex-col overflow-hidden">
        ...
        <div className="flex gap-2 border-t border-border p-3">
          <Button variant="secondary" className="h-10 w-10 px-0" aria-label="Attach"><Paperclip className="h-4 w-4" /></Button>
          <Input placeholder="Send message" value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") sendMessage(); }} />
          <Button className="h-10 w-10 px-0" onClick={sendMessage} aria-label="Send"><Send className="h-4 w-4" /></Button>
        </div>
      </section>
      ...
      <aside className="dashboard-surface space-y-4 p-5">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="secondary" onClick={() => patchActive({ mode: active?.mode === "AI" ? "Human" : "AI" })}>AI/Human</Button>
          <Button variant="secondary" onClick={() => patchActive({ pinned: !(active as ConversationRecord | undefined)?.pinned })}><Pin className="h-4 w-4" /> Pin</Button>
          <Button variant="secondary" onClick={() => patchActive({ archived: !(active as ConversationRecord | undefined)?.archived })}><Archive className="h-4 w-4" /> Archive</Button>
          <Button variant="secondary" onClick={() => patchActive({ assignedUserId: store.users[1]?.id })}>Assign</Button>
        </div>
        <label className="space-y-1">
          <span className="text-sm font-medium">Internal Notes</span>
          <Textarea value={active?.internalNotes ?? ""} onChange={(event) => patchActive({ internalNotes: event.target.value })} />
        </label>
        <label className="space-y-1">
          <span className="text-sm font-medium">Transfer To</span>
          <Select onChange={(event) => patchActive({ assignedUserId: event.target.value })}>
